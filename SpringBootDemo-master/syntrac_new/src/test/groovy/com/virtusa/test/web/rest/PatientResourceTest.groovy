...
class PatientResourceTest extends WebAppIntegrationBaseSpecification{
	...
	@Unroll
	def "Create a new patient and get the patient data" (firstName, lastName, dob, weight, mrn, gender, phone1, phone2, line1, line2, city, state, zip, email, maritalStatus, statusCode) {
		given:"I am an authenticated NMT"
		// TODO - Login flow
		def inputJson =  JsonOutput.toJson([firstName: firstName, lastName: lastName, dob: dob, weight: weight, mrn:mrn, gender: gender,
			phone1: phone1, phone2: phone2, line1: line1, line2:line2, city:city, state: state, zip: zip, email: email, maritalStatus: maritalStatus])
		...
		def response
		if(location)
			response = this.mockMvc.perform(get(location).contentType(MediaType.APPLICATION_JSON))
		then:
           if(response)
			response.andExpect(status().is(200))andExpect(jsonPath('$.firstName').value("First"))
