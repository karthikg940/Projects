...
public class ShippmentController {
	...
	public ResponseEntity<ShippmentEntity> create(
		...
		// return ResponseEntity.created(new URI("/api/shipment/" + id).body(link);
	...
	public ResponseEntity<ShippmentEntity> update(@PathVariable("id") long id,
		...
		else if(shipmentObject.getStatus().contains("Cancelled") && shippmentEntity.getStatus().contains("Delivered"))
		{
